...
except Exception:  # Fallback if requests isn't installed
...
class OllamaClient:
    ...
    def __init__(self):
        ...
        # For cloud services that require auth
        ...
        if self.use_remote:
            colored_print(
                f"  Mode: Remote API ({self.api_base_url})", Colors.YELLOW
            )
        else:
            colored_print(
                f"  Mode: Local CLI ({self.local_cmd})", Colors.YELLOW
            )
    ...
    def _check_local_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.local_cmd, "list"], capture_output=True, text=True, timeout=5
            )
    ...
    def _list_local_models(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                [self.local_cmd, "list"], capture_output=True, text=True
            )
    ...
    def _list_remote_models(self) -> List[str]:
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/api/tags", headers=self._get_headers()
            )
    ...
    def _generate_local(self, prompt: str, model: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                [self.local_cmd, "run", model, prompt],
                capture_output=True,
                text=True,
                timeout=120,
            )
    ...
    def _generate_remote(self, prompt: str, model: str, stream: bool = False) -> Dict[str, Any]:
        ...
        if requests is None:
            return {'success': False, 'response': '', 'error': 'requests not installed'}
        try:
            payload = {
                'model': model,
                'prompt': prompt,
