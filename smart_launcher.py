...
def launch_agent(role, name, method="terminal"):
    ...
    if method == "terminal":
        ...
        if is_wsl:
            print(f"⚠️WSL: No Windows terminal access, launching {role} agent '{name}' in background")
            print(f"Tip: Install Windows Terminal for better WSL experience")
        else:
            print(f"⚠️No terminal emulator found, launching {role} agent '{name}' in background")
...
def show_usage():
    ...
    print()
    print("💡 Examples:")
    print("  python3 smart_launcher.py ai-development")
...
def launch_custom_agents(agent_specs, method="terminal"):
    ...
    print("🛠️ Launching custom agent configuration")
